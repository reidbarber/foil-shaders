...
@MainActor public final class FoilShadersRenderer: NSObject {
  ...
  private let metalContext: FoilShadersMetalContext
  private var device: MTLDevice { metalContext.device }
  private var commandQueue: MTLCommandQueue { metalContext.commandQueue }
  private var textureLoader: MTKTextureLoader { metalContext.textureLoader }
  ...
  private var noiseTexture: MTLTexture? { metalContext.noiseTexture }
  private var fallbackNoiseTexture: MTLTexture? { metalContext.fallbackNoiseTexture }
  ...
  ///
  /// Renderers created with the same device share Metal setup resources that are safe to reuse.
  ///
  /// - Throws: `FoilShadersError.deviceError` if the device cannot create a command queue.
  public init(device: MTLDevice) throws {
    self.metalContext = try FoilShadersMetalContext.shared(for: device)
    super.init()
  ...
  init(context: FoilShadersMetalContext) {
  ...
  private func setupPipeline(fragmentFunctionName: String, library: MTLLibrary) throws {
    pipelineState = try metalContext.pipelineState(
