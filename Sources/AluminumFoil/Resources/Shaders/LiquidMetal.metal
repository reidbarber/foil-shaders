...
inline float getColorChanges(float c1, float c2, float stripe_p, float3 w, float blur, float bump, float tint, bool isImage) {
    ...
    ch = mix(ch, 1.0 - min(1.0, (1.0 - ch) / max(tint, 0.0001)), 0.0);
    return ch;
}
...
fragment float4 liquid_metal_fragment(VertexOutput in [[stage_in]],
    ...
    if (uniforms.u_isImage > 0.5) {
        ...
        if (uniforms.u_shape < 1.0) {
            ...
            float2 pixel_thickness = 250.0 / in.responsiveBoxGivenSize;
    ...
    if (uniforms.u_isImage > 0.5) {
        ...
        blur = uniforms.u_softness / 15.0 + 0.3 * edge;
    }

    float3 w = float3(thin_strip_1_width, thin_strip_2_width, wide_strip_ratio);
    w.y -= 0.02 * smoothstep(0.0, 1.0, edge + bump);
    float stripe_r = fract(direction + dispersionRed);
    float r = getColorChanges(color1.r, color2.r, stripe_r, w, blur + fwidth(stripe_r) + rExtraBlur, bump, uniforms.u_colorTint.r, uniforms.u_isImage > 0.5);
    float stripe_g = fract(direction);
    float g = getColorChanges(color1.g, color2.g, stripe_g, w, blur + fwidth(stripe_g) + gExtraBlur, bump, uniforms.u_colorTint.g, uniforms.u_isImage > 0.5);
    float stripe_b = fract(direction - dispersionBlue);
    float b = getColorChanges(color1.b, color2.b, stripe_b, w, blur + fwidth(stripe_b), bump, uniforms.u_colorTint.b, uniforms.u_isImage > 0.5);
